...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiDollarSign, FiTruck } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['poultry-farm-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SupplierInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/suppliers` : null),
    () => getSuppliers({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Supplier',
      path: isTenantUser ? `/suppliers/view/${data?.[0]?.id}` : '/suppliers',
      entity: 'supplier',
